...
def totals(where="", params=()):
    ...
    c = conn.cursor()
    c.execute(
        f"SELECT is_income, amount, hst_included FROM entries "
...
def hst():
    ...
    cur.execute("""
        ...
        WHERE e.is_income = 0
          AND e.hst_included = 1
        ORDER BY date(e.entry_date) DESC, e.id DESC
    """)
    rows = cur.fetchall()
    conn.close()
...
def expense_income():
    ...
    return render_template("expense_income.html",
                           title=APP_TITLE,
                           trucks=trucks,
                           drivers=drivers,
                           entries=entries)
...
def edit_entry(id):
    ...
    if request.method == "POST":
        ...
        cur.execute("""
            UPDATE entries
            SET entry_date=?, is_income=?, category=?, amount=?, hst_included=?,
                description=?, truck_id=?, driver_id=?
            WHERE id=?
        """, (
            entry_date,
            is_income,
...
def driver_pay():
    ...
    if request.method == "POST":
        ...
        where = (
            "is_income=0 AND category='Driver Pay' "
        ...
        if not items:
            ...
            conn.close()
            return render_template("driver_pay.html",
                                   title=APP_TITLE,
                                   drivers=drivers,
                                   items=[],
                                   pdf_path=None)
        ...
        c.setFont("Helvetica-Bold", 10)
        c.drawString(0.8*inch, y, "Date")
        c.drawString(2.0*inch, y, "Driver")
        c.drawString(4.0*inch, y, "Truck")
        c.drawRightString(7.7*inch, y, "Amount")
        y -= 0.15*inch
        c.setFont("Helvetica", 10)
        ...
        y -= 0.1*inch
        c.setFont("Helvetica-Bold", 12)
        c.drawRightString(6.9*inch, y, "Total Pay:")
        c.drawRightString(7.7*inch, y, f"${total:,.2f}")
        c.showPage()
        c.save()
